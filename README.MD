...
`0`: unlock the fn key \
`1`: lock the fn key, you can only trigger function with `Fn + F1-12` when you enabled \
BTW, you can press `Fn` key(light on) to reverse it.


## Smart charging
I don't know the meaning of arguments, but it is implemented, check \
